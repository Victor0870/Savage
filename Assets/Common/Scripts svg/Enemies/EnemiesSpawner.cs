...
namespace OctoberStudio
{
    ...
    public class EnemiesSpawner : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                var enemy = enemies[i];
                if (enemy == null || enemy.WaveOverride == null) continue;// theeemmmmmmmmmmmmmmm

                //Debug.Log($"Player: {PlayerBehavior.Player}, enemies count: {enemies?.Count}"); // theeemmmmmmmmmmmmmmm
                if (enemy.WaveOverride.DisableOffscreenTeleport) continue;
                if (PlayerBehavior.Player == null || PlayerBehavior.Player.transform == null) return;  // theeemmmmmmmmmmmmmmm
                ...
                if (PlayerBehavior.Player.LookDirection == Vector2.zero) continue; // theeemmmmmmmmmmmmmmm
