...
using OctoberStudio.Input; // <-- ĐÃ THÊM: Giải quyết lỗi InputType
...
namespace OctoberStudio.UI
{
    public class LobbyWindowBehavior : MonoBehaviour
    {
        ...
        // Khai báo chi phí Năng lượng và Module Currency
        private const string ENERGY_CODE = "EN";
        private const int ENERGY_COST = 8;
        ...
        {
            // BẮT ĐẦU QUÁ TRÌNH TRỪ EN VÀ KIỂM TRA ADS
            new PopupViewer().ShowLoadingPopup();
            CurrencyModule.SubtractCurrencyFromProfile(ENERGY_CODE, ENERGY_COST, OnEnergySubtracted);
        }

        private void OnEnergySubtracted(CBSUpdateCurrencyResult result)
        {
            new PopupViewer().HideLoadingPopup();

            if (result.IsSuccess)
            {
                // THÀNH CÔNG: ĐÃ TRỪ EN, TIẾN HÀNH KIỂM TRA ADS VÀ LOAD GAME
                ...
                Debug.Log("So lan bam Play: " + playCount);

                PlayerPrefs.SetInt("PlayCount", playCount);
                PlayerPrefs.Save();

                // Neu bam Play 3 lan thi hien thi quang cao
                if (playCount >= 3)
                {
                    Debug.Log("Hien thi quang cao sau 3 lan bam Play.");
                    ...
                    if (adManager != null)
                    {
                        // Gọi hàm hiển thị quảng cáo, và LoadStage() sẽ nằm trong callback của nó
                        adManager.ShowAdBeforeGameStart(() =>
                        {
                    ...
                    {
                        Debug.LogWarning("Khong tim thay AdManager! Bat dau game ngay lap tuc.");
                        GameController.LoadStage();
                    }
                }
                else
                {
                    // Neu chua du 3 lan, vao game ngay lap tuc
                    GameController.LoadStage();
                }
            }
            else
            {
                // THẤT BẠI: Xử lý lỗi từ Server
                var cbsError = result.Error;

                if (cbsError.CBSCode == ErrorCode.INSUFFICIENT_FUNDS)
                {
                    new PopupViewer().ShowSimplePopup(new PopupRequest
                    {
                        Title = "Lỗi Năng lượng",
                        Body = $"Không đủ {ENERGY_COST} {ENERGY_CODE} để chơi. Vui lòng nạp thêm."
                    });
                }
                else
                {
        ...
        private void ConfirmButtonClicked()
        {
            ...
            GameController.AudioManager.PlaySound(AudioManager.BUTTON_CLICK_HASH);
            // Không cần kiểm tra EN ở đây vì ConfirmButton là cho CONTINUING GAME (đã trả chi phí)
            GameController.LoadStage();
        }
