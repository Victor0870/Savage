...
namespace OctoberStudio.UI
{
    public class CharacterItemBehavior : MonoBehaviour
    {
        ...
        [SerializeField] GameObject[] startingAbilityObjects = new GameObject[2];
        [SerializeField] Image[] startingAbilityImages = new Image[2];
        ...
        public void Init(int id, CharacterData characterData, AbilitiesDatabase database)
        {
            ...
            */
            ...
            for (int i = 0; i < startingAbilityObjects.Length; i++)
            {
                if (i < characterData.StartingAbilities.Length)
                {
                    startingAbilityObjects[i].SetActive(true);
                    var abilityData = database.GetAbility(characterData.StartingAbilities[i]);
                    startingAbilityImages[i].sprite = abilityData.Icon;
                }
                else
                {
                    startingAbilityObjects[i].SetActive(false); // neu ko co skill 2
                }
            }
